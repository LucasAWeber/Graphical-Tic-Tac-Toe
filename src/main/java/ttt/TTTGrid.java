...
/**
 * This class works with the board/grid of classic TTT and its subsequent logic
 * @author Lucas Weber
 * @version 1.0
 */
public class TTTGrid extends boardgame.Grid{

    /**
     * This is the constructor for the TTTGrid class
     * @param wide width of board/grid
     * @param tall height of board/grid
     */
    public TTTGrid(int wide, int tall){
            super(wide,tall);
    }
    ...
    public ArrayList<Integer> parseStringIntoBoard(String toParse){
        ...
        while (i < toParse.length()) {
            if (toParse.charAt(i) == 'X' || toParse.charAt(i) == 'O') {
                setValue(j, k, String.valueOf(toParse.charAt(i)));
                j++;
                depth++;
